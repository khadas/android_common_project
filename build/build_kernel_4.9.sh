...
export ROOT_DIR=$(readlink -f $(dirname $0)/../../../../vendor/amlogic/common/kernel)
...
set -x
(cd ${OUT_DIR} && \
 make O=${OUT_DIR} ${CC_LD_ARG} -j$(nproc) $@ ${A32_GOALS} && \
 for aDts in ${KERNEL_DEVICETREE}; do \
   make O=${OUT_DIR} ${CC_LD_ARG} -j$(nproc) $aDts.dtb; \
 done)
set +x
...
if [ $KERNEL_A32_SUPPORT ]; then
	cp ${OUT_DIR}/arch/arm/boot/uImage ${ROOT_DIR}/${PRODUCT_DIRNAME}-kernel/4.9/
else
	cp ${OUT_DIR}/arch/arm64/boot/Image.gz ${ROOT_DIR}/${PRODUCT_DIRNAME}-kernel/4.9/
fi
...
if [ ${devicenum} == 1 ]; then
	echo "===single dts==="
	cp -f ${OUT_DIR}/arch/${ARCH}/boot/dts/amlogic/${KERNEL_DEVICETREE}.dtb ${ROOT_DIR}/${PRODUCT_DIRNAME}-kernel/4.9/${BOARD_DEVICENAME}.dtb
else
	echo "===multi dts==="
	mkdir -p ${OUT_DIR}/../multi_dts
	for aDts in ${KERNEL_DEVICETREE}; do
		cp ${OUT_DIR}/arch/${ARCH}/boot/dts/amlogic/$aDts.dtb ${OUT_DIR}/../multi_dts/
	done
	${DTBTOOL} -o ${ROOT_DIR}/${PRODUCT_DIRNAME}-kernel/4.9/${BOARD_DEVICENAME}.dtb -p ${OUT_DIR}/scripts/dtc/ ${OUT_DIR}/../multi_dts/
fi

echo "===build dtbo==="
${DTCTOOL} -@ -O dtb -o ${OUT_DIR}/${DTBO_DEVICETREE}.dtbo ${KERNEL_DIR}/arch/${ARCH}/boot/dts/amlogic/${DTBO_DEVICETREE}.dts
${DTIMGTOOL} create ${ROOT_DIR}/${PRODUCT_DIRNAME}-kernel/4.9/dtbo.img ${OUT_DIR}/${DTBO_DEVICETREE}.dtbo
