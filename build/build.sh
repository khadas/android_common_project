...
if [ $KERNEL_A32_SUPPORT ]; then
    ...
    if [ "$CONFIG_BOOTIMAGE" == "user" ];then
        cat ${KERNEL_DIR}/scripts/amlogic/meson64_a64_r_user_diffconfig >> ${KERNEL_DIR}/arch/arm64/configs/meson64_a64_gki_defconfig
    fi
...
if [ "$CONFIG_BUILTIN_MODULES" != true ];then
EXT_MODULES+="${KERNEL_DIR}/drivers/amlogic
${KERNEL_DIR}/sound/soc/amlogic
${KERNEL_DIR}/sound/soc/codecs/amlogic
"
fi
...
if [ ${devicenum} == 1 ]; then
    ...
    if [ $KERNEL_A32_SUPPORT ]; then
        if [ "${CUSTOMER_DTB}" == true ]; then
            cp -f ${OUT_DIR}/customer/arch/arm/boot/dts/${KERNEL_DEVICETREE}.dtb ${ROOT_DIR}/${PRODUCT_DIRNAME}-kernel/32/5.4/${BOARD_DEVICENAME}.dtb
        else
            cp -f ${OUT_DIR}/arch/arm/boot/dts/amlogic/${KERNEL_DEVICETREE}.dtb ${ROOT_DIR}/${PRODUCT_DIRNAME}-kernel/32/5.4/${BOARD_DEVICENAME}.dtb
        fi
    else
        if [ "${CUSTOMER_DTB}" == true ]; then
    ...
    if [ $KERNEL_A32_SUPPORT ]; then
        ${DTBTOOL} -o ${ROOT_DIR}/${PRODUCT_DIRNAME}-kernel/32/5.4/${BOARD_DEVICENAME}.dtb -p ${OUT_DIR}/scripts/dtc/ ${OUT_DIR}/../multi_dts/
    else
...
echo "===build dtbo==="
if [ $KERNEL_A32_SUPPORT ]; then
    ${DTCTOOL} -@ -O dtb -o ${OUT_DIR}/${DTBO_DEVICETREE}.dtbo ${KERNEL_DIR}/arch/arm/boot/dts/amlogic/${DTBO_DEVICETREE}.dts
else
    ${DTCTOOL} -@ -O dtb -o ${OUT_DIR}/${DTBO_DEVICETREE}.dtbo ${KERNEL_DIR}/arch/arm64/boot/dts/amlogic/${DTBO_DEVICETREE}.dts
fi
if [ $KERNEL_A32_SUPPORT ]; then
    ${DTIMGTOOL} create ${ROOT_DIR}/${PRODUCT_DIRNAME}-kernel/32/5.4/dtbo.img ${OUT_DIR}/${DTBO_DEVICETREE}.dtbo
else
...
if [ $KERNEL_A32_SUPPORT ]; then
RAMDISK_TARGET=${ROOT_DIR}/${PRODUCT_DIRNAME}-kernel/32/5.4/ramdisk/
else
...
for loop in `find ${COMMON_OUT_DIR}/${KERNEL_DIR}/drivers/amlogic/ -type f | grep "\.ko$"`
do
    cp $loop ${RAMDISK_TARGET}/lib/modules/
done

# copy extra ko to ramdisk
for loop in `find ${COMMON_OUT_DIR}/${KERNEL_DIR}/sound/soc/ -type f | grep "\.ko$"`
do
    cp $loop ${RAMDISK_TARGET}/lib/modules/
done
